...
namespace www.SoLaNoSoft.com.BearChessWin
{
    ...
    public partial class BearChessMainWindow : Window
    {
        ...
        {
            public UciInfo WhiteConfig { get; }
            public UciInfo BlackConfig { get; }
            public TimeControl TimeControl { get; }
            public string PlayerWhite { get; }
            public string PlayerBlack { get; }
            public bool StartFromBasePosition { get; }

            public CurrentGame(UciInfo whiteConfig, UciInfo blackConfig, TimeControl timeControl, string playerWhite, string playerBlack, bool startFromBasePosition)
            {
        ...
        public BearChessMainWindow()
        {
            ...
            _showMoves = _configuration.GetConfigValue("showMoves", "false").Equals("true");
            ...
            _showMaterial = _configuration.GetConfigValue("showMaterial", "false").Equals("true");
        ...
        private void ChessBoardUc_MakeMoveEvent(int fromField, int toField)
        {
            if (_pureEngineMatch)
            {
                _fileLogger?.LogDebug("ChessBoardUc_MakeMoveEvent Repaint board");
                chessBoardUcGraphics.RepaintBoard(_chessBoard);
                return;
            }
            ...
            if (_currentMoveIndex < _playedMoveList.Length)
            {
                ...
                foreach (var move in _chessBoard.GetPlayedMoveList())
                {
                    ...
                    _moveListWindow?.AddMove(move, string.Empty);
                }
            ...
            if (_chessBoard.CurrentColor == Fields.COLOR_WHITE)
            {
                ...
                if (_runningGame && !_pureEngineMatch)
                {
                    ...
                    {
                        if (!_pausedEngine) _engineWindow?.GoInfinite();
                    }
                }
        ...
        private void MenuItemWindowMoves_OnClick(object sender, RoutedEventArgs e)
        {
            if (_moveListWindow == null)
            {
                ...
                { 
                    ...
                    _moveListWindow.AddMove(move,isInCheck);
                }
            }
        }
        ...
        private void Engine_MakeMoveEvent(int fromField, int toField, string promote)
        {
            ...
            Dispatcher?.Invoke(() =>
            {
                _fileLogger?.LogDebug("Engine_MakeMoveEvent Repaint board");
                chessBoardUcGraphics.RepaintBoard(_chessBoard);
                var fenPosition = _chessBoard.GetFenPosition();
        ...
        {
            OpeningBookLoader.Init(_bookPath);
        }
        ...
        private void EChessBoardMoveEvent(string move)
        {
            ...
            if (_currentMoveIndex < _playedMoveList.Length)
            {
                ...
                foreach (var tmove in _chessBoard.GetPlayedMoveList())
                {
                    ...
                    _moveListWindow?.AddMove(tmove, string.Empty);
                }
                });
                _playedMoveList = new IMove[0];
                _currentMoveIndex = 0;
            ...
            Dispatcher?.Invoke(() =>
            {
                _fileLogger?.LogDebug("EChessBoard MakeMoveEvent Repaint board");
                chessBoardUcGraphics.RepaintBoard(_chessBoard);
                var fenPosition = _chessBoard.GetFenPosition();
                _bookWindows.ForEach(b =>
                {
