...
namespace www.SoLaNoSoft.com.BearChessWin
{
    ...
    public partial class BearChessMainWindow : Window
    {
        ...
        private string _prevEngineFenPosition;
        ...
        private bool _requestForHelpBegin = false;
        private bool _requestForHelp = false;
        ...
        private string[] _requestForHelpArray;
        private string[] _prevRequestForHelpArray;
        private bool _showRequestForHelp;
        private string _prevRequestForHelpFen;
        private bool _loadBuddyEngineOnGameStart;
        private bool _useBluetoothIChessOne;
        private bool _requestForHelpByEvent;
        private bool _showMoveLineChessnutAir;
        ...
        public BearChessMainWindow()
        {
            ...
            _synthesizer = new BearChessSpeech();
            ...
            {
                buttonConnect.ToolTip = "Connect to TabuTronic Cerno chessboard";
            }
            if (_lastEBoard.Equals(Constants.TabutronicSentio, StringComparison.OrdinalIgnoreCase))
            {
                buttonConnect.ToolTip = "Connect to TabuTronic Sentio chessboard";
            }
            ...
            imageChessnutAirShowMoveLine.Visibility = _showMoveLineChessnutAir ? Visibility.Visible : Visibility.Hidden;
            ChessnutAirLoader.Save(_configuration.FolderPath, _useBluetoothChessnutAir, _showMoveLineChessnutAir);
            ...
            _gamesPurePGNExport = bool.Parse(_configuration.GetConfigValue("gamesPurePGNExport", "false"));
...
//                _eChessBoard?.ShowMove(fromFieldFieldName, toFieldFieldName, promoteFigure,  prevMove?.GetMoveString(_eBoardLongMoveFormat, _displayCountryType));
            ...
            {
                string drawReason = _chessBoard.DrawByRepetition ? "Draw by position repetition" :
                                    _chessBoard.DrawByMaterial ? "Draw by insufficient material" :
                                    "Draw by 50 moves rule";
                if (_ficsClient != null && _ficsClient.IsLoggedIn)
                {
                    _ficsClient?.Send("draw");
                    MessageBox.Show($"{drawReason}. Request FICS for draw.", "Game finished", MessageBoxButton.OK,
        ...
        private void StartANewGame(bool runEngine = true)
        {
            ...
            _prevRequestForHelpArray = new []{string.Empty,string.Empty};
            if (_speechIsActive)
            {
            ...
            if (!_currentGame.StartFromBasePosition)
            {
                ...
                else
                {
                    ...
                    //if (_chessBoard.CurrentColor == Fields.COLOR_WHITE)
                    //{
                    //    _engineWindow?.SetFen(_chessBoard.GetFenPosition(),string.Empty, _currentGame.WhiteConfig.Name);
                    //}
                    //else
                    //{
                    //    _engineWindow?.SetFen(_chessBoard.GetFenPosition(),string.Empty, _currentGame.BlackConfig.Name);
                    //}
                }
            }
            else
            {
               _engineWindow?.SendToEngine("position startpos");
               // _engineWindow?.SendToEngine("position startpos",_currentGame.WhiteConfig.Name);
            }
            ...
            if (runEngine && _gameAgainstEngine)
            {
                ...
                else
                {
                    ...
                    _eChessBoard?.StopClock();
                    _eChessBoard?.DisplayOnClock("New game");
                }
        ...
        private void MenuItemGamesSave_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            var pgnCreator = new PgnCreator(_gameStartFenPosition, _gamesPurePGNExport);
            foreach (var move in _chessBoard.GetPlayedMoveList())
            {
        ...
        private void Engine_MakeMoveEvent(int fromField, int toField, string promote, decimal score, string bestLine, string engineName)
        {
            ...
            if (_chessBoard.MoveIsValid(fromField, toField))
            {
                ...
                if (prevMove!=null)
                {
                    ...
                    {
                        if (string.IsNullOrWhiteSpace(promote))
                            SpeakMove(fromFieldFigureId, toFieldFigureId, fromFieldFieldName, toFieldFieldName, FigureId.NO_PIECE, prevMove.GetMove(_chessBoard.EnemyColor).ShortMoveIdentifier);
                        else
                            SpeakMove(fromFieldFigureId, toFieldFigureId, fromFieldFieldName, toFieldFieldName, FigureId.FenCharacterToFigureId[promote], prevMove.GetMove(_chessBoard.EnemyColor).ShortMoveIdentifier);
                    });
            ...
            if (_chessBoard.DrawByRepetition || _chessBoard.DrawByMaterial || _chessBoard.DrawBy50Moves)
            {
                ...
                if (_ficsClient != null && _ficsClient.IsLoggedIn)
                {
                    ...
                    {
                        _ficsClient?.Send("draw");
                    MessageBox.Show($"Draw by {draw}. Request FICS for draw.", "Game finished", MessageBoxButton.OK,
                                    MessageBoxImage.Stop);
                    });
                }
                else
        ...
        {
            var pgnCreator = new PgnCreator(_gameStartFenPosition, false);
            foreach (var move in _chessBoard.GetPlayedMoveList())
            {
                pgnCreator.AddMove(move);
            }
        ...
        private void EngineWindow_EngineEvent(object sender, EngineEventArgs e)
        {
            ...
            if ((e.FirstEngine || _pureEngineMatch) && e.FromEngine.Contains(" pv "))
            {
                if (!e.FromEngine.Contains(" multipv ") || e.FromEngine.Contains(" multipv 1 "))
                {
                    ...
                    for (var i = 0; i < strings.Length; i++)
                    {
                        ...
                        if (!_buddyEngineLoaded && strings[i].Equals("score", StringComparison.OrdinalIgnoreCase))
                        {
            ...
            if (_eChessBoard != null && _requestForHelp )
            {
                if (_requestForHelpArray.Length > 0)
                {
                    try
                    {
                        Dispatcher?.Invoke(() =>
            ...
            if (_eChessBoard!=null && !e.FirstEngine && !_pureEngineMatch && e.FromEngine.Contains(" pv "))
            {
                if (!e.FromEngine.Contains(" multipv ") || e.FromEngine.Contains(" multipv 1 "))
                {
                    for (var i = 0; i < strings.Length; i++)
                    {
                        if (strings[i].Equals("pv"))
                        {
                            try
                            {
                                _requestForHelpArray = new[] { strings[i + 1].Substring(0, 2), strings[i + 1].Substring(2, 2) };
                                if (_prevRequestForHelpArray.Length > 0 && _requestForHelpArray.Length>0)
                                {
                                    if (_requestForHelpArray[0].Equals(_prevRequestForHelpArray[0]) && _requestForHelpArray[1].Equals(_prevRequestForHelpArray[1]))
                                    {
                                        _requestForHelpArray = Array.Empty<string>();
                                    }
                                }
                                if ((_requestForHelpByEvent || _requestForHelp) && _requestForHelpArray.Length>0)
                                {
                                    Dispatcher?.Invoke(() =>
                                    {
                                        _eChessBoard?.SetLedsFor(
        ...
        private void ConnectToUCB()
        {
            ...
            menuItemConnectToUCBBoard.Header = "Disconnect";
            menuItemMChessLink.IsEnabled = false;
            menuItemPegasus.IsEnabled = false;
            ...
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     ...
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
            textBlockEBoard.Text = $" Connected to UCB ({currentComPort})";
            ...
            imageDisconnect.Visibility = Visibility.Visible;
            _configuration.SetConfigValue("LastEBoard", _lastEBoard);
            buttonConnect.ToolTip = "Disconnect from UCB";
            _eChessBoard.Calibrate();
            if (_currentAction == CurrentAction.InRunningGame)
            {
                _eChessBoard.SetFen(_chessBoard.GetFenPosition(), string.Empty);
        ...
        private void ConnectToCitrine()
        {
            ...
            _usedEngines[Constants.Citrine].AddOptionValue("setoption name Level Play value " + _eChessBoard.Level[0]+ _eChessBoard.Level[1]);
            _usedEngines[Constants.Citrine].AddOptionValue("setoption name Level 1-8 value " + _eChessBoard.Level[3]);
            menuItemConnectToCitrineBoard.Header = "Disconnect";
            menuItemMChessLink.IsEnabled = false;
            menuItemPegasus.IsEnabled = false;
            menuItemSquareOffPro.IsEnabled = false;
            menuItemDGTEBoard.IsEnabled = false;
            ...
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     ...
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
            textBlockEBoard.Text = $" Connected to Citrine ({currentComPort})";
            imageBT.Visibility = Visibility.Hidden;
            ...
            imageDisconnect.Visibility = Visibility.Visible;
            _configuration.SetConfigValue("LastEBoard", _lastEBoard);
            buttonConnect.ToolTip = "Disconnect from Citrine";
            _eChessBoard.Calibrate();
            if (_currentAction == CurrentAction.InRunningGame)
            {
                _eChessBoard.SetFen(_chessBoard.GetFenPosition(), string.Empty);
        ...
        private void ConnectToOSA()
        {
            ...
            menuItemConnectToOSA.Header = "Disconnect";
            menuItemMChessLink.IsEnabled = false;
            menuItemPegasus.IsEnabled = false;
            menuItemSquareOffPro.IsEnabled = false;
            menuItemDGTEBoard.IsEnabled = false;
            ...
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     ...
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
            textBlockEBoard.Text = $"Connected to OSA ({currentComPort} with {_eChessBoard.GetCurrentBaud()} Baud)";
            imageBT.Visibility = Visibility.Hidden;
        ...
        private void ConnectToChessnutAir()
        {
            _fileLogger?.LogInfo("Connect to Chessnut Air/Pro chessboard");
            ...
            menuItemConnectToChessnutAirBoard.Header = "Disconnect";
            menuItemMChessLink.IsEnabled = false;
            menuItemCertabo.IsEnabled = false;
            menuItemPegasus.IsEnabled = false;
            menuItemSquareOffPro.IsEnabled = false;
            ...
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     ...
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
            textBlockEBoard.Text = $"Connected to Chessnut Air/Pro ({currentComPort})";
            ...
            imageDisconnect.Visibility = Visibility.Visible;
            _configuration.SetConfigValue("LastEBoard", _lastEBoard);
            buttonConnect.ToolTip = "Disconnect from Chessnut Air/Pro";
            _eChessBoard.Calibrate();
            if (_currentAction == CurrentAction.InRunningGame)
            {
                _eChessBoard.SetFen(_chessBoard.GetFenPosition(), string.Empty);
        ...
        private void ConnectToIChessOne()
        {
            ...
            menuItemConnectToIChessOneBoard.Header = "Disconnect";
            menuItemMChessLink.IsEnabled = false;
            menuItemCertabo.IsEnabled = false;
            menuItemPegasus.IsEnabled = false;
            menuItemSquareOffPro.IsEnabled = false;
            ...
            menuItemNovagBoard.IsEnabled = false;
            menuItemChessnutAirBoard.IsEnabled = false;
            menuItemConfigureIChessOne.IsEnabled = false;
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     _currentAction == CurrentAction.InEasyPlayingMode ||
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
        ...
        private void ConnectToDGT()
        {
            ...
            menuItemConnectToDGTEBoard.Header = "Disconnect";
            menuItemMChessLink.IsEnabled = false;
            menuItemPegasus.IsEnabled = false;
            menuItemSquareOffPro.IsEnabled = false;
            menuItemCertabo.IsEnabled = false;
            ...
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     ...
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
            textBlockEBoard.Text = $"Connected to DGT e-Board ({currentComPort})";
            imageBT.Visibility = currentComPort.Equals("BT", StringComparison.OrdinalIgnoreCase)
                                     ? Visibility.Visible
                                     : Visibility.Hidden;
            ...
            imageDisconnect.Visibility = Visibility.Visible;
            _configuration.SetConfigValue("LastEBoard", _lastEBoard);
            buttonConnect.ToolTip = "Disconnect from DGT e-Board";
            _eChessBoard.Calibrate();
            if (_currentAction == CurrentAction.InRunningGame)
            {
                _eChessBoard.SetFen(_chessBoard.GetFenPosition(), string.Empty);
        ...
        private void DisconnectFromEBoard(MenuItem menuItem, string boardName)
        {
            ...
            buttonConnect.ToolTip = $"Connect to {boardName}";
        ...
        private void ConnectToTabutronicSentio()
        {
            ...
            menuItemConnectToSentio.Header = "Disconnect";
            menuItemMChessLink.IsEnabled = false;
            menuItemPegasus.IsEnabled = false;
            menuItemSquareOffPro.IsEnabled = false;
            menuItemDGTEBoard.IsEnabled = false;
            ...
            _eChessBoard.SetDemoMode(_currentAction == CurrentAction.InAnalyseMode ||
                                     ...
                                     _currentAction == CurrentAction.InGameAnalyseMode);
            var currentComPort = _eChessBoard?.GetCurrentComPort();
            textBlockEBoard.Text = $"Connected to TabuTronic Sentio chessboard ({currentComPort})";
            imageBT.Visibility = currentComPort.Equals("BT", StringComparison.OrdinalIgnoreCase)
                                     ? Visibility.Visible
                                     : Visibility.Hidden;
            ...
            imageDisconnect.Visibility = Visibility.Visible;
            _configuration.SetConfigValue("LastEBoard", _lastEBoard);
            buttonConnect.ToolTip = "Disconnect from TabuTronic Sentio chessboard";
            if (_currentAction == CurrentAction.InRunningGame)
            {
                _eChessBoard.SetFen(_chessBoard.GetFenPosition(), string.Empty);
            }
        ...
        private void ConnectToTabutronicCerno()
        {
            ...
            menuItemConnectToCerno.Header = "Disconnect";
            menuItemMChessLink.IsEnabled = false;
            menuItemPegasus.IsEnabled = false;
            menuItemSquareOffPro.IsEnabled = false;
            menuItemDGTEBoard.IsEnabled = false;
            ...
            imageDisconnect.Visibility = Visibility.Visible;
            _configuration.SetConfigValue("LastEBoard", _lastEBoard);
            buttonConnect.ToolTip = "Disconnect from TabuTronic Cerno chessboard";
            if (_currentAction == CurrentAction.InRunningGame)
            {
                _eChessBoard.SetFen(_chessBoard.GetFenPosition(), string.Empty);
            }
        ...
        {            
            ...
            if (_currentAction == CurrentAction.InRunningGame && !_currentGame.TimeControl.TournamentMode)
            {
                ...
                {
                    _prevRequestForHelpFen = _prevEngineFenPosition;
                    if (!_requestForHelpBegin && !_requestForHelp)
                    {
                        _requestForHelpBegin = _showRequestForHelp;
                    }
            ...
            {
                if (!string.IsNullOrWhiteSpace(_prevRequestForHelpFen) && _prevEngineFenPosition.Equals(_prevRequestForHelpFen))
                //{
                ...
                //else
                {
                    _requestForHelp = _requestForHelpBegin;
                    if (_requestForHelp)
                    {
                        _fileLogger.LogDebug("Request for help detected");
                    }

                    if (_eChessBoard != null && (_requestForHelp || _requestForHelpByEvent) && _requestForHelpArray != null &&
                        _requestForHelpArray.Length > 0)
                    {
                        try
                        {
                ...
                _requestForHelpBegin = false;
                _prevEngineFenPosition = fenPosition;
            }
        ...
        private void DuelWindowCloneDuelSelected(object sender, int duelId)
        {
            ...
            if (_currentDuel != null)
            {
                ...
                var engineMatchWindow = new NewEngineDuelWindow(_configuration, _database, _currentDuel.AdjustEloBlack || _currentDuel.AdjustEloWhite, _currentDuel.StartFromBasePosition) { Owner = this };
        ...
        private void DuelWindowContinueDuelSelected(object sender, int duelId)
        {
            ...
            if (_currentDuel != null)
            {
                ...
                foreach (var databaseGameSimple in _database.GetDuelGames(_currentDuelId))
                {
                    ...
                    {
                        LoadAGame(_database.LoadGame(databaseGameSimple.Id, false));
        ...
        private void DuelWindow_RepeatGameSelected(object sender, int gameId)
        {
            ...
            if (_currentDuel != null)
            {
                ...
                var databaseGame = _database.LoadGame(gameId, false);
                databaseGame.Reset();
        ...
        private void TournamentInfoWindow_StopTournament(object sender, EventArgs e)
        {
            ...
            _tournamentInfoWindow?.Close();
           var pgnCreator = new PgnCreator(false);
           foreach (var move in _chessBoard.GetPlayedMoveList())
           {
               pgnCreator.AddMove(move);
           }
        ...
        private void MenuItemConnectFics_OnClick(object sender, RoutedEventArgs e)
        {
            if (_ficsClient != null)
            {
                ...
                _ficsWindow?.Close();
                _ficsWindow = null;
                menuItemConnectFics.Header = "Connect to FICS";
            ...
            try
            {
                ...
                //_ficsClient.Connect();
                menuItemConnectFics.Header = "Disconnect from FICS";
            ...
            {
                _ficsClient = null;
                MessageBox.Show(ex.Message, "FICS", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void MenuItemShowMoveLineChesssnutAir_OnClick(object sender, RoutedEventArgs e)
        {
            _showMoveLineChessnutAir = !_showMoveLineChessnutAir;
            _configuration.SetConfigValue("showmovelineChessnutAir", _showMoveLineChessnutAir.ToString());
            imageChessnutAirShowMoveLine.Visibility = _showMoveLineChessnutAir ? Visibility.Visible : Visibility.Hidden;
            ChessnutAirLoader.Save(_configuration.FolderPath, _useBluetoothChessnutAir, _showMoveLineChessnutAir);
        }
        ...
        private void MenuItemEngineLoadBuddy_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show($"Open 'Load & manage engines' dialog to define a buddy engine.", "No buddy engine found", MessageBoxButton.OK,
                                MessageBoxImage.Stop);
                return;
            }
            LoadBuddyEngine();
 ...
 //           _useBluetoothIChessOne = !_useBluetoothIChessOne;
            ...
            imageChessnutAirBluetooth.Visibility = _useBluetoothIChessOne ? Visibility.Visible : Visibility.Hidden;
            IChessOneLoader.Save(_configuration.FolderPath, _useBluetoothIChessOne);
        }
        ...
        private void MenuItemConfigureIChessOne_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            var winConfigure = new WinConfigureIChessOne(_configuration) { Owner = this };
            winConfigure.ShowDialog();
        }
        ...
        {
            {
                ...
                winConfigureSquareOff.ShowDialog();
                if (reConnect)
                {
                    ConnectToSquareOffPro();
                }
        ...
        private void MenuItemConfigBearChess_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            winConfigureBearChess.Owner = this;
