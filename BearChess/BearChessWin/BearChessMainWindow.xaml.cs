    ...
    public partial class BearChessMainWindow : Window
    {
        ...
        private void ChessBoardUc_MakeMoveEvent(int fromField, int toField)
        {
            ...
            if (isInCheck.Equals("#"))
            {
                ...
                _eChessBoard?.SetAllLedsOff();
                _eChessBoard?.StopClock();
                _eChessBoard?.ShowMove(fromFieldFieldName, toFieldFieldName, prevMove?.GetMoveString(_eBoardLongMoveFormat));
                _engineWindow?.Stop();
                _chessClocksWindowBlack?.Stop();
                _chessClocksWindowWhite?.Stop();
        ...
        private void SetTimeControl()
        {
            ...
            HandlingClocks.SetClocks(_timeControl, _chessClocksWindowWhite, _chessClocksWindowBlack,
        ...
        private void ContinueAGame(bool runEngine = true)
        {
            ...
            if (_gameAgainstEngine)
            {
                ...
                {
                    _engineWindow =  EngineWindowFactory.GetEngineWindow(_configuration, _uciPath);
        ...
        private void StartANewGame(bool runEngine = true)
        {
            ...
            if (runEngine && _gameAgainstEngine)
            {
                ...
                if (_timeControl.TimeControlType == TimeControlEnum.TimePerGame ||
                    _timeControl.TimeControlType == TimeControlEnum.TimePerGameIncrement ||
                    _timeControl.TimeControlType == TimeControlEnum.TimePerMoves)
                {
                    if (_chessClocksWindowWhite != null)
                    {
        ...
        private void ReadInstalledEngines()
        {
            try
            {
                ...
                //if (!_usedEngines.ContainsKey(ficsUciInfo.Name))
                //{
                //    _usedEngines.Add(ficsUciInfo.Name, ficsUciInfo);
                //}
        ...
        private void Engine_MakeMoveEvent(int fromField, int toField, string promote, decimal score, string bestLine, string engineName)
        {
            ...
            if (isInCheck.Equals("#"))
            {
                ...
                Dispatcher?.Invoke(() =>
                {
                    ...
                    _eChessBoard?.ShowMove(fromFieldFieldName, toFieldFieldName,
                                           _chessBoard.GetPrevMove()?.GetMoveString(_eBoardLongMoveFormat));
            ...
            if (_chessBoard.DrawByRepetition || _chessBoard.DrawByMaterial || _chessBoard.DrawBy50Moves)
            {
                ...
                Dispatcher?.Invoke(() =>
                {
                    ...
                    chessBoardUcGraphics.RepaintBoard(_chessBoard);
                    _eChessBoard?.SetAllLedsOff();
                    _eChessBoard?.ShowMove(fromFieldFieldName, toFieldFieldName, _chessBoard.GetPrevMove()?.GetMoveString(_eBoardLongMoveFormat));
                    _moveListWindow?.AddMove(engineMove, _gameAgainstEngine && _timeControl.TournamentMode);
                    _moveListWindow?.RemainingMovesFor50MovesDraw(_chessBoard.RemainingMovesFor50MovesDraw);
        ...
        private void DisconnectFromPegasus()
        {
            ...
            DisconnectFromEBoard();
            buttonConnect.ToolTip = "Connect to DGT Pegasus";
            menuItemConnectToDGTEBoard.Header = "Connect";
        }

        private void DisconnectFromChessnutAir()
        {
            _fileLogger?.LogInfo("Disconnect from Chessnut Air");
            DisconnectFromEBoard();
            buttonConnect.ToolTip = "Connect to Chessnut Air";
            menuItemConnectToChessnutAirBoard.Header = "Connect";
        }
        ...
        private void EChessBoardAwaitedPositionEvent(object sender, EventArgs e)
        {
            ...
            if (_timeControl != null && _currentAction == CurrentAction.InRunningGame)
            {
                ...
                Dispatcher?.Invoke(() =>
                {
                    ...
                    if (_chessBoard.CurrentColor == Fields.COLOR_WHITE)
                    {
                        ...
                        if (_timeControl.WaitForMoveOnBoard)
                        {
                            _chessClocksWindowWhite?.Go();
                        }
        ...
        private void MenuItemConnectFics_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                _ficsWindow = new FicsWindow(_ficsClient, new FileLogger(Path.Combine(_ficsPath, "ficsWindow.log"), 10, 10));
                _ficsWindow.NewGameEvent += ficsWindow_NewGameEvent;
                _ficsWindow.Show();
            }
            catch (Exception ex)
            ...
            var timeControl = new TimeControl()
                              {
                                  ...
                                  TimeControlType = TimeControlEnum.TimePerGame, TournamentMode = false, Value1 = time1, Value2 = time2,
