...
namespace www.SoLaNoSoft.com.BearChessBase
{
    ...
    public class UciInfo
    {
        ...
        [XmlIgnore]
        public bool SupportChangeMultiPV { get; set; }
        ...
        public bool CanMultiPV()
        {
            
                var uciElo = OptionValues.FirstOrDefault(f => f.StartsWith("setoption name MultiPV"));
                return uciElo != null;
            
            
        }

        public int GetConfiguredMultiPV()
        {
            if (IsPlayer)
            {
                return 1;
            }
            var multiPV = OptionValues.FirstOrDefault(f => f.StartsWith("setoption name MultiPV"));
            if (multiPV != null)
            {
                {
                
                    {
                        var strings = multiPV.Split(" ".ToCharArray());
                        if (int.TryParse(strings[strings.Length - 1], out int pvValue))
                        {
                            return pvValue;
                        }
                    }
