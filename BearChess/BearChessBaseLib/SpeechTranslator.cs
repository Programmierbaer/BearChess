...
namespace www.SoLaNoSoft.com.BearChessBase
{
    public static class SpeechTranslator
    {
        ...
        public static string GetFigureName(int figureId, string langCode, Configuration configuration)
        {
            ...
            var fideRules = configuration.GetBoolValue("blindUserSayFideRules", true);
            if (fideRules)
            {
              //  result = FigureId.FigureIdToFideName[figureId];
            }
            if (langCode.Contains("de"))
            {
            ...
            if (bool.Parse(configuration.GetConfigValue("SpeechOwnLanguage", "false")))
            {
                switch (figureId)
                {
                    case FigureId.WHITE_KING:
                        figureName = configuration.GetConfigValue("SpeechKing", string.Empty);
                        break;
                    case FigureId.WHITE_QUEEN:
                        figureName = configuration.GetConfigValue("SpeechQueen", string.Empty);
                        break;
                    case FigureId.WHITE_ROOK:
                        figureName = configuration.GetConfigValue("SpeechRook", string.Empty);
                        break;
                    case FigureId.WHITE_BISHOP:
                        figureName = configuration.GetConfigValue("SpeechBishop", string.Empty);
                        break;
                    case FigureId.WHITE_KNIGHT:
                        figureName = configuration.GetConfigValue("SpeechKnight", string.Empty);
                        break;
                    case FigureId.WHITE_PAWN:
                        figureName = configuration.GetConfigValue("SpeechPawn", string.Empty);
                        break;
        ...
        public static string GetCastleKingsSide(string langCode, Configuration configuration)
        {
            ...
            var fideRules = configuration.GetBoolValue("blindUserSayFideRules", true);
            fideRules = false;
            if (fideRules || langCode.Contains("de"))
            {
