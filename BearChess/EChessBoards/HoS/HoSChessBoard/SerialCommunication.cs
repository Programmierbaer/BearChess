...
namespace www.SoLaNoSoft.com.BearChess.HoSChessBoard
{
    public class SerialCommunication : AbstractSerialCommunication
    {
        ...
        protected override void Communicate()
        {
            ...
            int sleepTime = _useBluetooth ? 2500 : 5;
            while (!_stopReading)
            {
                try
                {
        ...
        private string ConvertFromUSBRead(byte[] bArray)
        {
        ...
        private string ConvertFromBTRead(byte[] bArray)
        {
            string r = string.Empty;
            ...
            while (!_stopReading)
            {
                ...
                try
                {
                    ...
                    if (withConnection && !_pauseReading)
                    {
                        ...
                        if (_useBluetooth && !readLine.Contains(","))
                        {
                            continue;
                        }
                        if (_useBluetooth && readLine.StartsWith("FF"))
                        {
                            readLine = string.Empty;
                            continue;
                        }
                        if (_useBluetooth && !readLine.StartsWith("0"))
                        {
                            readLine = "00" + readLine;
                        }
