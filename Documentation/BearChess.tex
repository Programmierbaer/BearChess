...
BearChess supports \textbf{Certabo} chessboards, chessboards connected via \textbf{Millennium} ChessLink or Millennium eOne, the \textbf{DGT} e-Boards or Pegasus chessboard, \textbf{Square Off Pro}, \textbf{Chessnut Air}, \textbf{NOVAG} Citrine/UCB,
and \textbf{Saitek} boards with an OSA interface (Galileo, Leornado, Renaissance).\\
...
\end{figure}
\begin{itemize}
	\item \includegraphics[scale=0.2]{bearchess.png} \textbf{Start a new game} opens a new window to select opponents and time control (see chapter \textbf{\ref{SelectOpponent}  \nameref{SelectOpponent}} on page \pageref{SelectOpponent}).
...
BearChess automatically recognizes whether you are playing with the white or black pieces, depending on how you set up the pieces in the base position (except for DGT Pegasus,Square Off Pro, NOVAG and Saitek boards).
...
You get the biggest advantage if you have connected an electronic chessboard with a piece recognition. Millennium eOne, DGT Pegasus, Square Off Pro, NOVAG boards and Saitke bords has not. Connect your electronic chessboard before you start the analysis mode.\\ 
...
\begin{itemize}
	\item Fix: UCI configuration values for Komodo Dragon.
\end{itemize}

\subsection*{Version 0.9.2.1 $\Rightarrow$  0.9.3.0}
\begin{itemize}
	\item New: Support from NOVAG Citrine and NOVAG UCB board.
	\item New: Play against NOVAG Citrine as UCI engine.
	\item New: Support from Saitek Galileo/Leonardo/Renaissance boards.
	\item New: Play against Saitek boards as UCI engine.
	\item New: Separate time control for white and black.
	\item New: Continue a game from a given position.
\end{itemize}

\subsection*{Version 0.9.2.0 $\Rightarrow$  0.9.2.1}
\begin{itemize}
	\item Fix: engine move output for analysing engines
\end{itemize}

\subsection*{Version 0.9.1.0 $\Rightarrow$  0.9.2.0}
\begin{itemize}
	\item New: Improved engine move output
	\item New: Chessmen included designed by \textbf{Bryan Whitby}
	\item New: Chessmen and fields included designed by \textbf{Certabo}
\end{itemize}

\subsection*{Version 0.9.1.0 $\Rightarrow$  0.9.1.1}
\begin{itemize}
	\item Fix: Error on connection via bluetooth 
\end{itemize}
