...
\begin{document}
	...
	\begin{abstract}
        ...
        Many thanks to the programmers of the chess engines that I have integrated into BearChess
	 	\begin{itemize}
	 	  ...
	 	  \item \textbf{Wasp 7.07} by John Stanback (\url{https://waspchess.stanback.net/index.html}).
	...
	\begin{enumerate}
		...
		\item Play with \textbf{TabuTronic Cerno, Sentio, Cerno Spectrum} or \textbf{Tactum} chessboards.
	...
	Optionally, the design can also be changed so that most windows remain within the application window (more like a Multiple Doument Interface). Read more in chapter \textbf{\ref{ConfigureBearChess}  \nameref{ConfigureBearChess}} on page \pageref{ConfigureBearChess}.\\\\
	...
	\end{figure}
	\begin{itemize}
		\item \includegraphics[scale=0.6]{BearChessIcon.png} \textbf{Start a new game} opens a new window to select opponents and time control (see chapter \textbf{\ref{SelectOpponent}  \nameref{SelectOpponent}} on page \pageref{SelectOpponent}).
		
		\item \includegraphics[scale=0.5]{robot.png} \textbf{Analysis} allows you to analyze games or positions with suppport of several chess engines (see chapter \textbf{\ref{AnalyzeMode}  \nameref{AnalyzeMode}} on page \pageref{AnalyzeMode})
		...
		\item \includegraphics[scale=0.1]{FicsLogo.png} connects you with Free Internet Chess Server (FICS). See chapter \textbf{\ref{FICS}  \nameref{FICS}} on page \pageref{FICS}.	
	...
	\begin{itemize}
		...
		\item \textbf{Show analysis bar} shows the current rating of the buddy engine. Above the center line White has the advantage, below it Black. It is always displayed from White's point of view.
			\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{AnalysisBar1.png}
			\caption{Analysis Bar with advantage for White}
			\label{fig:AnalysisBar1}
		\end{figure}
	...
	\begin{itemize}
		...
		\item \textbf{Show evaluation symbols} \; =\; +/=\; +/-\; +-\; -/=\; -/+\; -+\; instead of the numerical values.
	...
	\end{itemize}
	BearChess can handle Polyglot, Arena and CTG opening books. Read more on chapter \textbf{\ref{OpeningBooks}  \nameref{OpeningBooks}} on page \pageref{OpeningBooks}.
	...
	\end{figure}
	\begin{itemize}
		\item  \includegraphics[scale=0.6]{BearChessIcon.png} \textbf{BearChess} opens a window where you can change many BearChess basic settings.  Read more on chapter \textbf{\ref{ConfigureBearChess}  \nameref{ConfigureBearChess}} on page \pageref{ConfigureBearChess}.
		...
		\begin{figure}[H]
			...
			\label{fig:Player}
		\end{figure}	
		\item  \includegraphics[scale=0.5]{sound.png}  \textbf{Voice/Sounds} opens a dialog where you can configure some tones that sound when the engine makes a move.
		
		\item \includegraphics[scale=0.1]{FicsLogo.png} configure your connection parameters for Free Internet Chess Server (FICS). See chapter \textbf{\ref{FICS}  \nameref{FICS}} on page \pageref{FICS}.		
	\end{itemize}
	...
	\begin{itemize}
		...
		\item  \includegraphics[scale=0.05]{tabutronic_logo_def.png} \textbf{TabuTronic} opens a window where you can configure and connect to TabuTronic chessboards (Cerno and Sentio). Read more on chapter \textbf{\ref{ConfigureTabuTronicCerno}  \nameref{ConfigureTabuTronicCerno}} on page \pageref{ConfigureTabuTronicCerno},chapter \textbf{\ref{ConfigureTabuTronicCernoSpectrum}  \nameref{ConfigureTabuTronicCernoSpectrum}} on page \pageref{ConfigureTabuTronicCernoSpectrum}, chapter \textbf{\ref{ConfigureTabuTronicSentio}  \nameref{ConfigureTabuTronicSentio}} on page \pageref{ConfigureTabuTronicSentio} or \textbf{\ref{ConfigureTabuTronicTactum}  \nameref{ConfigureTabuTronicTactum}} on page \pageref{ConfigureTabuTronicTactum}.	
		\item  \includegraphics[scale=0.1]{iChessOneIcon.png} \textbf{iChessOne} opens a window in which you can configure and connect to iChessOne chessboards.  Read more on chapter \textbf{\ref{ConfigureIChessOne}  \nameref{ConfigureIChessOne}} on page \pageref{ConfigureIChessOne}.    
	...
	{\color{red}\textbf{*}} Currently, a BearChess engine is used in conjunction with an iChessOne, Chessnut, Certabo, TabuTronic Cerno or Millennium ChessLink chessboard (see  \textbf{\ref{BuddyBearChessEngines}  \nameref{BuddyBearChessEngines}} on page \pageref{BuddyBearChessEngines}).
	...
	\begin{figure}[H]
		\centering
		\includegraphics[scale=0.7]{LoadEngine2.png}
...
Certabo requires two configuration steps. The used COM port and a calibration to detect the chess pieces. If your PC supports bluetooth and you own the Bluetooth module from Certabo, you can also connect to the Certabo board via Bluetooth or Bluetooth LE. If you want to do this, activate the "Bluetooth Classic" option or, if you have the new Bluetooth module with BTLE support, first activate the "Bluetooth LE" option before opening the configuration window.\\
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{Cerno1.png}
...
\subsection{Configure TabuTronic Cerno Spectrum} \label{ConfigureTabuTronicCernoSpectrum}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{CernoSpectrum1.png}
	\caption{Open a window to configure TabuTronic Cerno Spectrum}
	\label{fig:CernoSpectrum1}
\end{figure}

Spectrum requires two configuration steps. The used COM port and a calibration to detect the chess pieces. If your PC supports bluetooth and you own the Bluetooth module from Certabo, you can also connect to the Spectrum board via Bluetooth or Bluetooth LE. If you want to do this, activate the "Bluetooth Classic" option or, if you have the new Bluetooth module with BTLE support, first activate the "Bluetooth LE" option before opening the configuration window.\\


\begin{figure}[H]
	\centering
	\includegraphics[scale=1.0]{CernoSpectrum2.png}
	\caption{Configure TabuTronic Cerno Spectrum}
	\label{fig:CernoSpectrum2}
\end{figure}
...
The Spectrum offers multi-colored LEDs. This allows a much more detailed configuration. Each LED can combine three colors ({\color{red}red}, {\color{ForestGreen}green} and {\color{blue}blue}) in different proportions (0 = none to 255 = full).\\
The following example defines the LED for the square from which you are to take the chess pieces, with {\color{ForestGreen}green} and {\color{blue}blue}.
The LED for the target square is {\color{ForestGreen}green}.\\
You can activate the checkbox for “Flash”. However, this only has an effect on the “Move from” and “Move to” fields, which then flash alternately.

\begin{figure}[H]
	\centering
	\includegraphics[scale=1.0]{CernoSpectrum3.png}
	\caption{Set LEDs }
	\label{fig:CernoSpectrum3}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=1.0]{CernoSpectrum6.png}
	\caption{Test your LEDs }
	\label{fig:CernoSpectrum6}
\end{figure}
The icon changes to \includegraphics[scale=0.4]{eye_half.png} and small \includegraphics[scale=0.3]{eye.png} appears besides every configuration option. Click on the small eyes shows you an example of the LED configuration on your board (with the exception of the “Flash” options).
...
\subsection{Configure Chessnut Go} \label{ConfigureChessnutGo}
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{ChessnutGo1.png}
	\caption{Open a window to configure Chessnut Go}
	\label{fig:ChessnutGo1}
\end{figure}

The configuration is exactly as it is for the Chessnut Air boards.
Read more on chapter \textbf{\ref{ConfigureChessnutAir}  \nameref{ConfigureChessnutAir}} on page \pageref{ConfigureChessnutAir}.
...
\subsection{Accepts subsequent corrections}

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{NewGame4.png}
	\caption{Accepts subsequent corrections}
	\label{fig:NewGame4}
\end{figure}

If two people are playing against each other, i.e. no engine is selected as the opponent, this option can be activated. In this case, the last move can be corrected until the opponent has made his reply move. This mode is intended for beginners who may want to correct their move again.
Even the mistake often made by beginners when castling - moving the rook first - is automatically corrected. \\
{\color{red}\textbf{*}} However, this option is only available when playing on an electronic chessboard with real piece recognition.
...
\subsection{Cerno Spectrum: board orientation}
The figure recognition assumes that the board is aligned so that the USB port is on the bottom left.


\subsection{TabuTronic Cerno/Spectrum: Calibration}
For the first time, the engine assumes that all chessmen are on their initial position and the extra queens on d3 (white queen) and d6 (black queen).
...
\begin{itemize}			
	    \item Improvements for the boards without real piece recognition.
...
\subsection*{Version 1.1.1.1 $\Rightarrow$  1.2.0.0}
\begin{itemize}			
	\item {\color{blue}New}: Support for TabuTronic Cerno Spectrum.
	\item {\color{blue}New}: Analysis bar.
	\item {\color{blue}New}: Transfer of the buddy engine ratings to the game comments.
	\item {\color{blue}New}: Accepts subsequent corrections on player vs player games.
	\item {\color{teal}Changes}: Improvements display of best move lines.
	\item {\color{teal}Changes}: Improvements for visually impared player and TabuTronic Tactum.
	\item {\color{teal}Changes}: Improvements for free analysis mode.
\end{itemize}
...
\subsection*{Version 1.1.0.0 $\Rightarrow$  1.1.1.0}
\begin{itemize}			
