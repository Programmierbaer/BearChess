...
BearChess supports Certabo chessboards, chessboards connected via Millennium ChessLink and the DGT Pegasus chessboard.\\
...
I am a professional programmer and write the software in my spare time and it is free. I am not an employee of Inventhio Srl trading (Certabo) or Millennium 2000 GmbH or Digital Game Technology B.V. If something does not work, Certabo, Millennium or DGT is not responsible for it.\\
...
If you click on the BearChess main screen, all other open windowss are alo grought to the foreground. If you hold down the Shift key and move the BearChess main screen, all other windows will also be moved synchronously.\\\\
...
\subsection{Info}
Opens a windows where you can easily go to the path where all configuration is stored.
...
If they set a countdown greater than 0 seconds, the confirmation is automatic.
...
If an engine needs special parameters or a parameter to be recognized as a UCI engine, you can use a CMD file instead of the EXE file. Write all required command into the CMD file and install.
...
\begin{itemize}
	...
	\item \includegraphics[scale=0.5]{file_extension_wav.png} Opens a files selection dialog for an individuel sound file.
...
BearChess supports three electronic chessboards: Certabo boards, Millennium boards via the ChessLink module and DGT Pegasus. Certabo and Millennium boards communicates via an USB port as serial COM port or via Bluetooth. DGT Pegasus communications only via Bluetooth. Which COM port is used is not the same for all computers and can change over time, especially if you have more than one COM port available on your computer.\\
...
In the classic variant, coupling is necessary and the connection is then made to a serial COM port. The LE variant is even more user-friendly. No coupling is necessary and there is no conversion to a COM port.
...
For your Millennium ChessLink you may need to configure the COM port or Bluetooth used. You can also change how the LEDs should light up.\\
...
The figure recognition assumes that the board is aligned so that the cable is on right.
...
Unlike the other chessboards, the DGT Pegasus requires a Bluetooth LE connection. Make sure that your Bluetooth adapter also supports LE.
...
After a successful connection, you need to confirm that the figures are all in the right place.
...
BearChess automatically recognizes whether you are playing with the White or Black pieces, depending on how you set up the pieces in the base position (except for DGT Pegasus).
...
\includegraphics[scale=0.4]{cog.png} opens the engine configuration dialog. This is the same dialog as for load and manage engines. \textbf{However, all changes are only used until you close BearChess you explicit reload the engine.}\\
...
You get the biggest advantage if you have connected an electronic chessboard with a piece recognition. DGT Pegasus has not. Connect your electronic chessboard before you start the analysis mode.\\ 
...
The small board starts with the current position. When a DGT Pegasus board is connected, the behavior is as if no board is connected.
...
If a DGT Pegasus is connected, they do not need to disconnect. You will be prompted at the end to place the figures correctly. \\
...
\subsection{Millennium ChessLink: board orientation}
